...
describe Guard::Watcher do
  ...
  describe ".guards_matching_files?" do
    before(:all) do
    ...
    describe "with at least on watcher that match a file given" do
      specify { Guard::Watcher.guards_matching_files?(@guards, ['lib/my_wonderful_lib.rb', 'guard_rocks_spec.rb']).should == [@guard1] }
    end

    describe "with no watcher matching a file given" do
      specify { Guard::Watcher.guards_matching_files?(@guards, ['lib/my_wonderful_lib.rb']).should == [] }
    end
  end
